...
def callback_button(update: Update, context: CallbackContext):
    ...
    admin_list = list(set([OWNER_ID] + list(DEV_USERS)))
    ...
    if query_type == "db_leave_chat":
        if query.from_user.id in admin_list:
            ...
            query.answer("You are not allowed to use this.", show_alert=True)
        ...
        if query.from_user.id in admin_list:
            ...
            reply = (
                f"Cleaned up {invalid_chat_count} chats and "
                f"{invalid_gban_count} gbanned users from DB."
            )
